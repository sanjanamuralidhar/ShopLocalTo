...
//up[date chating session
  ...
  static Future<dynamic> getVendorMessage({id: 0}) async {
    await Future.delayed(Duration(seconds: 1));
    final result = await httpManager.post(
        url: 'http://dev.shoplocalto.ca/api/message-list?id=' + id.toString());
    print('$result');
  ...
  static Future<ProductDetailTabPageModel> getLoadTabDetail(
      {id: 0, tabExtend: false}) async {
    await Future.delayed(Duration(seconds: 1));
    String path =
  ...
  // sanjana
  static Future<List<TabModel>> getTabtabDetail(
      {id: 0, tabExtend: false}) async {
    await Future.delayed(Duration(seconds: 1));
    final result = await httpManager.post(
        url: 'http://dev.shoplocalto.ca/api/vendortabs?id=' + id.toString());
...
// non token api
  static Future<List<MyLocation>> getSuggestedLocation() async {
    await Future.delayed(Duration(seconds: 1));
    final result =
        await httpManager.post(url: 'http://dev.shoplocalto.ca/api/locations');
